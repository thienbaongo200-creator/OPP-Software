...
namespace QuanLyThuVien.Services
{
    public class MuonTraService
    {
                ...
                try
                {
                    ...
                    foreach (string maSach in unique)
                    {
                        using (SqlCommand check = new SqlCommand(@"
                            SELECT SoLuongHienCo FROM DauSach WITH (UPDLOCK,HOLDLOCK)
                    ...
                    foreach (string maSach in unique)
                    {
                        ...
                        using (SqlCommand update = new SqlCommand(@"
                            UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo-1
        ...
        {
            return Db.Query(@"
                SELECT ct.MaChiTiet, pm.MaPhieuMuon, pm.MaDocGia,
            ...
            string tinhTrang, decimal phiPhat)
        {
            ...
            using (SqlTransaction tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                ...
                SELECT ct.MaDauSach, ct.NgayTraThucTe, pm.NgayMuon, pm.NgayHenTra
                ...
                WHERE MaChiTiet=@MaCT AND NgayTraThucTe IS NULL", cn, tx))
                    ...
                    // Nếu sách không mất/hư hỏng thì cộng lại vào tồn kho
                    if (!mat && !huHong)
                    {
                        using (SqlCommand stock = new SqlCommand(@"
                    UPDATE DauSach SET SoLuongHienCo=SoLuongHienCo+1 WHERE MaDauSach=@Ma", cn, tx))
                        {
                    ...
                    // Nếu có phạt thì lập phiếu phạt
                    if (canPhat)
                    {
