...
namespace QuanLyThuVien.Services
{
    public class DocGiaService
    {
        ...
        {
            return Db.Query(@"
SELECT dg.MaDocGia, dg.Ho, dg.Ten, dg.NgaySinh, dg.Phai, dg.SoDienThoai,
...
ORDER BY dg.MaDocGia");
        ...
        public KetQuaXuLy Luu(DocGia dg, bool capNhat)
        {
            ...
            if (!Regex.IsMatch(dg.Email, @"^[^@\s]+@[^@\s]+\.[^@\s]+$"))
            ...
            try
            {
                string sql = capNhat
                    ? @"UPDATE DocGia 
                        SET Ho=@Ho, Ten=@Ten, NgaySinh=@NgaySinh, Phai=@Phai, SoDienThoai=@SDT,
                            DiaChi=@DiaChi, Email=@Email, Anh3x4=@Anh 
                        WHERE MaDocGia=@Ma"
                ...
                int n = Db.Execute(sql,
                    ...
                    new SqlParameter("@SDT", string.IsNullOrEmpty(dg.SoDienThoai) ? DBNull.Value : (object)dg.SoDienThoai),
                    new SqlParameter("@DiaChi", dg.DiaChi.Trim()),
                    new SqlParameter("@Email", dg.Email.Trim()),
                    new SqlParameter("@Anh", string.IsNullOrEmpty(dg.Anh3x4) ? DBNull.Value : (object)dg.Anh3x4)
                );
        ...
        public KetQuaXuLy GiaHanThe(string maDocGia, DateTime hanMoi, bool daDongLePhi)
        {
                ...
                ORDER BY HanSuDung DESC", cn))
            {
                ...
                using (SqlDataReader rd = cmd.ExecuteReader())
                {
                    ...
                    DateTime hanCu = rd.GetDateTime(1);
