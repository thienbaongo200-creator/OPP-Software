...
namespace QuanLyThuVien.Forms
{
    public partial class FrmMuonTra : Form
    {
        ...
        {
            if (dgvSachCon.CurrentRow?.DataBoundItem is DataRowView r)
            {
                if (selectedBooks.Rows.Count >= 3)
                {
                    MessageBox.Show("Chỉ được chọn tối đa 3 đầu sách khác nhau.");
                    return;
                }
                ...
                foreach (DataRow row in selectedBooks.Rows)
                {
                    ...
                    {
                        MessageBox.Show("Đầu sách này đã có trong danh sách mượn.");
                        return;
                    }
                }
